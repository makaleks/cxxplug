...
#include "cxxplug_gen_collected_proxies/args.hpp"
#include "cxxplug_gen_collected_proxies/gen_collection.hpp"
...
int main (int argc, char **argv) {
    ...
    if (
        Args::is_argv_wrong(
            argc, argv,
            &output_dir_name, &interface_file_path_arg, &proxies,
            &is_verbose
        )
    ) {
        cout << Args::get_help();
    ...
    Args args(
        argv + is_verbose, argc - is_verbose,
        output_dir_name, move(proxies),
        is_verbose
    );
    ...
    try {
        ...
        CollectionGenerationResult result = generateCollectionFiles(
